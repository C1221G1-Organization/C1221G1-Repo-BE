...
public interface IMedicineRepository extends JpaRepository<Medicine, String> {
    ...
    */
    ...
    @Query(value = "select medicineId,medicineName,medicinePrice,medicineImage, sum(totalQuantity) as soldQuantity " + "from "
        ...
        + "m.medicine_image as medicineImage," + "sum(cd.cart_detail_quantity) as totalQuantity " + "from medicine m\n"
        + "         inner join cart_detail cd on m.medicine_id = cd.medicine_id " + "         inner join cart c on cd.cart_id = c.cart_id "
        + "         inner join medicine_type mt on m.medicine_type_id = mt.medicine_type_id " + "where c.cart_status = 1 " + "group by m.medicine_id "
        + "union " + "select m.medicine_id as medicineId," + "       m.medicine_name as medicineName,"
        + "       (m.medicine_import_price * (1+m.medicine_retail_sale_profit/100)/m.medicine_conversion_rate) as medicinePrice,"
        + "       m.medicine_image as medicineImage," + "       sum(im.invoice_medicine_quantity) as totalQuantity " + "from medicine m\n"
        + "         inner join invoice_medicine im on m.medicine_id = im.medicine_id "
        + "         inner join medicine_type mt on m.medicine_type_id = mt.medicine_type_id " + "group by m.medicine_id " + ") as total "
        + "group by medicineId " + "order by soldQuantity desc " + "limit 10;", nativeQuery = true)
    ...
    */
    ...
    @Query(value = "select m.medicine_id as medicineId, m.medicine_name as medicineName,"
        + "(m.medicine_import_price * (1+m.medicine_retail_sale_profit/100)/m.medicine_conversion_rate) as medicinePrice,"
        + "m.medicine_manufacture as medicineManufacture, " + "m.medicine_image as medicineImage," + "mt.medicine_type_name as medicineTypeName "
        + "from medicine m inner join medicine_origin mo on m.medicine_origin_id = mo.medicine_origin_id "
        ...
        + " order by case when :sort = 'priceDesc' then medicinePrice end desc, case when :sort = 'priceAsc' then medicinePrice end asc, case when :sort = 'idDesc' then medicineId end desc", countQuery =
        "select m.medicine_id as medicineId, m.medicine_name as medicineName,"
            + "(m.medicine_import_price * (1+m.medicine_retail_sale_profit/100)/m.medicine_conversion_rate) as medicinePrice,"
            + "m.medicine_manufacture as medicineManufacture, " + "m.medicine_image as medicineImage," + "mt.medicine_type_name as medicineTypeName "
            + "from medicine m inner join medicine_origin mo on m.medicine_origin_id = mo.medicine_origin_id "
            + "inner join medicine_type mt on m.medicine_type_id = mt.medicine_type_id"
            + " where m.medicine_name like concat('%',:name,'%') and m.medicine_type_id = :typeId and m.flag=1"
            + " order by case when :sort = 'priceDesc' then medicinePrice end desc, case when :sort = 'priceAsc' then medicinePrice end asc, case when :sort = 'idDesc' then medicineId end desc", nativeQuery = true)
    ...
    */
    ...
    @Query(value = "select m.medicine_id as medicineId, m.medicine_name as medicineName,"
        ...
        + "m.medicine_manufacture as medicineManufacture, " + "m.medicine_image as medicineImage," + "mt.medicine_type_name as medicineTypeName "
        + "from medicine m inner join medicine_origin mo on m.medicine_origin_id = mo.medicine_origin_id "
        + "inner join medicine_type mt on m.medicine_type_id = mt.medicine_type_id" + " where m.medicine_name like concat('%',:name,'%') and m.flag=1"
        + " order by case when :sort = 'priceDesc' then medicinePrice end desc, case when :sort = 'priceAsc' then medicinePrice end asc, case when :sort = 'idDesc' then medicineId end desc", countQuery =
        "select m.medicine_id as medicineId, m.medicine_name as medicineName,"
            + "(m.medicine_import_price * (1+m.medicine_retail_sale_profit/100)/m.medicine_conversion_rate) as medicinePrice,"
            + "m.medicine_manufacture as medicineManufacture, " + "m.medicine_image as medicineImage," + "mt.medicine_type_name as medicineTypeName "
